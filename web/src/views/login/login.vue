<template>
  <div>
  <!-- 有样式的登录页面 -->
  <div v-if="hasLoginStyle" class="relative min-h-screen flex">
    <div class="
                    flex flex-col
                    sm:flex-row
                    items-center
        ...
        <div class="max-w-md w-full mx-auto space-y-8">
          ...
          <div class="mt-8 space-y-6" >
            ...
            <div style="font-weight: 500;font-size: 13px;letter-spacing: 1px;">
              <span>账号：用户名:{{demoAccount}}   密码：{{demoPassword}}</span><br/><br/>
...
<!--              <span> <a target="_blank" style="text-decoration: underline;" href="http://8.146.211.120:8086/"> 完整版演示 http://8.146.211.120:8086/</a></span><br/><br/>-->
              <span> <a target="_blank" style="text-decoration: underline;" :href="mobileUrl"> 移动端登陆 {{mobileUrl}}</a></span><br/><br/>
              <span> <a target="_blank" style="text-decoration: underline;" :href="giteeUrl"> gitee {{giteeUrl}}</a></span>
...
export default {
  ...
  data() {
    ...
    return {
      loginLogo: '',
      loginForm: {
        username: process.env.VUE_APP_DEMO_ACCOUNT,
      ...
      logoShow:false,
      title:'',
      demoAccount: process.env.VUE_APP_DEMO_ACCOUNT,
      demoPassword: process.env.VUE_APP_DEMO_PASSWORD,
      mobileUrl: process.env.VUE_APP_MOBILE_URL,
      giteeUrl: process.env.VUE_APP_GITEE_URL
    }
  },
  watch: {
    $route: {
  ...
  computed: {
    hasLoginStyle() {
      return process.env.VUE_APP_LOGIN_STYLE === 'true'
    }
  },
  mounted() {
